...
void main() {
  ...
  group('ClosureAnnotation', () {
    ...
    test('gives multiple line comment when it it does not fit on one line', () {
      expect(new ClosureAnnotation(returnType: stringType, paramTypes: {'foo': numberType}).toString(),
          "/**\n"
          " * @param {number} foo\n"
          " * @return {string}\n"
          " */");
    ...
    test('inserts indentation', () {
      expect(new ClosureAnnotation(returnType: stringType, paramTypes: {'foo': numberType})
          .toString("  "),
    ...
    test('compresses @type, @final, @const, @private, @protected, @typedef', () {
      ...
      expect(new ClosureAnnotation(type: stringType, isFinal: true).toString(),
          "/** @final {string} */");
      expect(new ClosureAnnotation(type: stringType, isPrivate: true).toString(),
          "/** @private {string} */");
      expect(new ClosureAnnotation(type: stringType, isTypedef: true).toString(),
          "/** @typedef {string} */");
      expect(new ClosureAnnotation(type: stringType, isProtected: true).toString(),
          "/** @protected {string} */");
      expect(new ClosureAnnotation(type: stringType, isPrivate: true, isConst: true, isFinal: true, isProtected: true, isTypedef: true).toString(),
